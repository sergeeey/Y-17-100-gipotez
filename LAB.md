...
**Следующие кандидаты:**
- **H.** V2' (detrend-then-surrogate): убрать сглаженный сезонный тренд ПЕРЕД генерацией суррогата
  (AR(1) или IAAFT) на остатке, вернуть тренд после. Независимо мотивирован ДВУМЯ провалившимися
  стационарными нулями (H-B3-1c и H-B3-1d), а не изобретён ad hoc. Один новый эксперимент ID
  (Minimal Relaxation Rule).
- **F.** Phase 2 H-B3-1 (≥3 коллапса) — гейтится на исходе H
