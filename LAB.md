...
| B3 May 1972 ↔ TDA | H-B3-1 Phase 1 (Peter Lake) | **BLOCKED** (Substrate Gate) | EDI закрыл доступ (PASTA API 403, портал → Cloudflare Turnstile), CAPTCHA не обходил. `substrate_gate.md`. Не свидетельство против гипотезы — просто другой источник (см. ниже) |
| B3 | **H-B3-1b Phase 1 (O'Brien lakes) — реально выполнен** | **LEAD** (2026-09-06) | Тот же bridge, реально доступный источник (O'Brien 2023, GitHub). Lower Zurich: TDA опередил EWS на +24 мес (верное направление). НО оба негативных контроля (Windermere, Loch Leven) дали ложное срабатывание — и AR(1)-суррогат без всякого механизма пересекает тот же порог в 80–83% случаев → **CRITERION_INVALID** (порог tau≥0.5 никогда не различал сигнал от шума на этой длине ряда), не провал гипотезы. Rescue Review: `weak_alive`, 3 конкретных дешёвых следующих шага в `decision.md`. `experiments/20260906-may1972-tda-ews-obrienlakes/` |
