...
| B3 May 1972 ↔ TDA | H-B3-1 Phase 1: TDA лидирует classical EWS на Peter Lake? | **BLOCKED** (Substrate Gate, 2026-09-06) | Источник данных исправлен и найден точно (Carpenter 2011, пакет `knb-lter-ntl.360.2`), но EDI закрыл публичный доступ: PASTA REST API → 403 на все методы, веб-портал → Cloudflare Turnstile. 9 путей проверено, ни один не задействовал обход CAPTCHA. НЕ свидетельство против гипотезы — `substrate_gate.md`. Нужен: (a) пользователь скачивает вручную, (b) EDI API-токен, (c) другой открытый датасет |
