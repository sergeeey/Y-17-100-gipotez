...
## Point 91 (2026-09-18) — "vertex-support universality" as evidence toward `(SP)`: KILLED. First draft contained a composite-`n` test-design error AND a numerical threshold bug that together manufactured a false "sub-floor" finding; both independently re-confirmed, not accepted on the skeptic's word alone

**Context.** Per Point 90's own Step (a)/(b), and the user's explicit "го все по очереди автономно," attempted the cheapest of the two named next steps: check whether this project's own `s=2Q+1` KKT identity (Point 85) — established only for the `θ`-OPTIMAL vertex — actually holds for EVERY non-degenerate vertex of the SAME feasible polytope (not just the one maximizing `θ`). If so, this would be new, cheap, genuine evidence toward a vertex-restricted analogue of the paper's own `(SP)` sparsity conjecture.

**First-draft claim (now retracted).** Dimension-counting argument: a non-degenerate vertex in `R^f` (`f=|free|` generators) has exactly `f` active constraints among the `m+1` inequalities `y_k≥0`; if `k=0` is not among them, support`=2Q+1`, else `2Q+2`. Empirically "confirmed" via 450 random-objective LP solves (`n=63,127,251`) plus an adversarial unit-vector/Cauchy-tailed search (~660 more trials): reported support never below `2Q+1−6`, concentrated at `{2Q+1,2Q+2}`. Conclusion drawn: "essentially all vertices of this specific polytope have support `Θ(n)` at rate `≈1/2` — pilot evidence toward a vertex-restricted, fixed-graph-instance `(SP)`."

**Sent to a real, context-asymmetric skeptic agent** (given only the claim, the analytic argument, and the raw numeric summary — no session history). **Verdict: `FALSIFIED`**, via two independent, fully specified, checkable technical findings. Both re-derived here from scratch with fresh code, NOT accepted on the skeptic's report alone:

1. **`n=63` is composite (`63=3²×7`), independently verified (`sympy.isprime`).** Tao's uncertainty principle (the entire theoretical basis for `s≥2Q+1`) is a `Z_p` (PRIME modulus) statement — using it at a composite `n` is a test-design error, not a minor detail. The skeptic constructed an EXPLICIT feasible counterexample: the subgroup `H={0,21,42}⊂Z_63` (order 3), `x:=1_H`. **Independently re-verified, fresh code (`verify_n63_subgroup.py`): at this project's own seed-42 `n=63` instance, generator `g=21` is confirmed FREE (`bits[20]=0`), the constructed `x` is genuinely feasible (`y≥0` everywhere, checked directly), and `support(y)=21`** — 6 below the claimed universal floor `2Q+1=27`. **This project's own 450+660-trial adversarial search — including all `2f` unit-vector objectives — never found this vertex.** The reason, per the skeptic's own diagnosis, independently plausible and not separately re-derived here: random/generic-direction objective sampling selects vertices with probability proportional to their normal cone's solid angle, and sparse/degenerate vertices generically have vanishingly small normal cones — the search method is structurally blind to exactly the vertices that matter for a sparsity conjecture, not merely under-sampled.

2. **A DEVASTATING, independently-confirmed numerical bug: the reported "support=125 at `n=251` (6 below the floor `2Q+1=131`)" is a pure threshold artifact, not a real sub-floor vertex.** `n=251` is prime — Point 90's own established Tao-floor fact (`s≥2Q+1` for ANY feasible point, not just vertices, already on record in this project since 2026-09-15) says support **cannot** legitimately fall below `131` here. Independently re-ran the EXACT objective (`e_54-`) that produced the "125" result (`verify_n251_support_bug.py`) and inspected the raw `y_k` values at multiple thresholds: **at threshold `1e-6`, support=125; at threshold `1e-8` and every tighter threshold tested (`1e-10`, `1e-12`, `1e-14`), support=131 — EXACTLY `2Q+1`, matching the established floor with zero slack.** The smallest 15 `|y_k|` values, inspected directly, show a clean gap: true-zero coordinates sit at `~1e-17` to `~1e-18` (floating-point noise floor), while the 6 "missing" coordinates were genuinely nonzero at a scale between `1e-8` and `1e-6` — my own `1e-6` threshold (used throughout the original check scripts) was too loose and silently mis-classified real, small-but-nonzero support as zero, manufacturing a false sub-floor result in EXACTLY the direction the whole investigation was hoping to find. **This bug plausibly explains the `n=63`-adjacent "26" and `n=251`-adjacent "129" outlier results in the original distribution check too — not separately re-verified here, but the mechanism is identical and the direction (artificial undercounting) matches.**

**Third finding, conceptual not numerical (also independently sound on inspection, not re-derived from scratch but checked for internal consistency): the whole test was run in a regime where the question is TRIVIAL even if the numerics had been clean.** Edge density `0.5` gives `E[Q]≈m/2≈n/4`, so the Tao floor itself is already `≈n/2` — already ruling out any `cn`-sparse vector for essentially ANY constant `c<1/2`, by Tao's inequality ALONE, with no vertex-specific argument needed at all. The paper's own `(SP)` conjecture has content specifically in the LOW-density regime (small `c`) where the floor itself is much weaker than the conjecture — none of the three tested instances (`n=63,127,251`, density `0.5`) touch that regime. Separately: since (per finding 1's own counterexample-search argument) the MINIMUM-support feasible point of a polytope of this type is generically achieved AT a vertex, "restricting to vertices" does not actually weaken the statement relative to "all feasible points" — the "vertex-restricted version" framing in the original draft implied a deliberately-weakened, more tractable sub-claim, when in fact no real weakening was achieved; the actual weakenings (fixed graph instead of the random ensemble, density `0.5` instead of small `c`) were real but never named as such.

### Kill Analysis (per FL's Anti-Overfitting Gate — required for REJECT)

**What is killed:** the specific claim "empirical search across random+adversarial objectives shows this polytope's vertices resist sparsity below `~2Q+1`, providing pilot evidence toward `(SP)`" — killed on BOTH legs: the search method is structurally blind to the sparse vertices that would matter (confirmed by an explicit, feasible, found-by-construction counterexample at `n=63`), and the specific "found a sub-floor vertex" numeric claim at `n=251` was a threshold bug, not a real result (confirmed by re-deriving the SAME point at tighter thresholds and getting exactly the established floor, not a violation of it).

**What is NOT killed, and was already known:** for PRIME `n`, Tao's uncertainty principle already gives `support(y)≥2Q+1` for EVERY feasible point (not just vertices) — this project's own activeContext.md has stated this since 2026-09-15 (Point 64/85's own context). The `n=127` and `n=251` instances, once correctly thresholded, sit EXACTLY on this already-known floor — a clean, boring, zero-novelty confirmation of an already-established fact, not new information.

**Revival condition, stated explicitly (Adaptive Iteration discipline):** this specific investigation is dead as run. A genuinely informative version would need: (a) LOW-density instances (small `c`, e.g. `p∈{0.05,0.1,0.2}`) at prime `n`, where the Tao floor is much weaker than any candidate `(SP)`-type statement, so there is real room for a violation to show up; (b) a search method that does not rely on generic-direction LP objectives — e.g. explicit construction from subgroup/coset structure (as the skeptic's own `n=63` counterexample did, deliberately, not by chance), since random objectives are shown here to be structurally blind to exactly the sparse vertices that matter; (c) any numeric support-counting must use a verified-clean threshold (established here: floating-point true-zero sits around `1e-17`, real nonzero values are typically many orders of magnitude larger — a threshold of `1e-8` or tighter, with an explicit gap check like the one run here, not an unexamined fixed cutoff).

**Methodology lesson, `[REPEAT]`-worthy for this project's OWN future numeric investigations, not just this one:** three distinct, independently-real errors compounded in one first draft — (1) a composite modulus used where the theory requires prime, (2) a support-counting threshold never checked against the actual floating-point gap structure of the specific computation, (3) testing in a parameter regime (density `0.5`) where the target question is analytically trivial regardless of what the numerics say. None of these three would have been caught by re-running the SAME check again — each required either independent theoretical re-derivation (primality, the Tao floor) or inspecting raw intermediate values at multiple thresholds, exactly the kind of check a real, context-asymmetric skeptic pass supplies and self-review does not. This is the `[REPEAT×10]`-level pattern already logged in this project's memory: every substantive new numeric claim sent to a real skeptic this window has found at least one genuine, load-bearing error.

## Point 92 (2026-09-18) — Point 90's Step 2 (`mean(CV²)` drift vs. Bandeira's `O(log³n)` ceiling): the claimed "novel Parseval connection" is FALSIFIED as novel — it is already explicit in this project's own primary derivation file — but a small, genuine, corrected result survives: a SIMPLER, unconditional bound `CV²≤n‖y‖²−1`, closing activeContext.md's own flagged question with a clean (if modest) yes

**Context.** The other of Point 90's two named cheap next steps: check whether `mean(CV²)`'s empirically-observed drift (Point 87) is already implied by Bandeira et al.'s own `O(log³n)`-type bound (Point 79) — flagged repeatedly in this project's own `activeContext.md` as "the most promising untried direction." First-draft claim: `E‖x*‖²` (Bandeira's bounded quantity) and `n‖y*‖₂²` (this project's own Point 64 quantity) are the SAME object via Parseval, a "previously-unmade explicit connection" that, combined with Point 64's `n‖y‖²=(n/s)(1+CV²)` identity and the empirical fact that `n/s` is non-drifting, gives `CV²(y*)=O(log³n)` — closing the flagged question, plus a "headroom" argument (anchoring an implied constant `C` at the `n=127` data point) claiming the proven ceiling permits `~6.3×` more growth than the `~1.06×` actually observed across `n=127→8009`.

**Sent to a real, context-asymmetric skeptic** (claim + derivation only, no session history). **Verdict: `WEAKENED`**, on grounds independently re-checked here against the PRIMARY canonical source before acceptance, not taken on the skeptic's word:

**(1) `[FALSIFIED as novel]` — the "new connection" is not new; it is ALREADY EXPLICIT, verbatim, in this project's own foundational derivation file.** Read `codex-20260914-susceptibility/CROSS_TURAN_ENERGY_THEORY.md` directly (the file Point 79 itself cites as the source of `E‖x*‖²=O(log³n)`): it states outright, **"With `y=FFT(x)/n`, Parseval gives `‖x‖₂²=n‖y‖₂²`. Thus the one live analytic target is `sup_n E‖x*(G)‖₂² = sup_n n E‖y*(G)‖₂² < infinity. (POL)`"** — the exact identity claimed as a first-draft discovery here, already written down, already used to STATE `(POL)` itself, well before this point. The skeptic's diagnosis is correct and independently confirmed: connecting "`E‖x*‖²`" and "`n‖y*‖²`" is not bridging two independently-known facts, it is restating a step that was already load-bearing inside the project's own existing derivation. This is a genuine overclaim, corrected here, not merely caveated.

**(2) `[CONFIRMED, simplifies the original argument]` — the original derivation's reliance on the empirical fact "`n/s` is non-drifting" was unnecessary, and dropping it also resolves a separate, valid skeptic concern about needing JOINT (not marginal) moment control.** From Point 64's exact identity `n‖y‖²=(n/s)(1+CV²)` and the TRIVIAL, unconditional fact `s≤n` (hence `n/s≥1`): `(n/s)(1+CV²)≥1+CV²`, so **`CV²(y) ≤ n‖y‖²−1`, for EVERY instance, with no assumption on `n/s`'s behavior at all.** Taking expectations (valid by linearity, no joint-moment argument needed): `E[CV²] ≤ E[n‖y‖²]−1 ≤ C²log³n−1`. This is simpler than the original draft's route (which needlessly decomposed `n‖y‖²` into `n/s` and `CV²` separately, then invoked their empirical joint behavior) — and it sidesteps the skeptic's valid objection that `n/s` empirics were being used exactly where Point 64's own verdict ("`BOTH-STABLE-PILOT`, NOT moment control... renaming it conditioning or leverage is not progress") warned against overreaching. The corrected chain needs no such move.

**(3) `[RESOLVED, primary source checked]` — the skeptic's concern that Lemma 5's guarantee is "with high probability," not almost-surely, and that this gap was never checked, turns out to already be correctly handled in the SAME primary file.** `CROSS_TURAN_ENERGY_THEORY.md` states Lemma 5 holds "with probability at least `1−1/n`," and its own derivation explicitly closes the tail: "their stronger proof-line estimate **and the deterministic fallback `n‖y‖₂²≤n`** imply `E‖x*‖²=O(log³n)`" — i.e., on the `1/n`-probability bad event, the trivial ceiling `n‖y‖²≤n` contributes at most `(1/n)·n=O(1)` to the expectation, safely inside the claimed `O(log³n)` budget. Not a new derivation here — just confirming, by reading the primary source directly rather than assuming, that this concern is already addressed, not a live gap.

**(4) `[CORRECTED, C-free reformulation]` — the original "headroom" claim ("the proven ceiling permits CV² up to `6.3`") was stated as if `C` were known; it is not — the `n=127` data point only gives a LOWER bound on any consistent `C`, not its true value, so an absolute number like `6.3` cannot be asserted as "proven."** The qualitative point survives cleanly once restated `C`-free: **the ratio `(log(8009)/log(127))³ ≈ 6.39` holds for ANY constant `C`, since `C` cancels** (re-verified, `check_headroom_c_free.py`) — i.e., the `O(log³n)` ceiling permits roughly `6.4×` more RELATIVE growth from `n=127` to `n=8009` than is actually observed (`≈1.06×`, Point 87's own established, bootstrap-confirmed, statistically significant `9σ+` drift) — a genuine, large, qualitative gap independent of the unknown constant, not the specific absolute number `6.3` from the first draft.

**(5) `[CLARIFIED, Gate 1 — artifact identity]` — the skeptic's "non-monotonic, dip at the middle point" concern does not apply to the dataset actually used here.** Re-checked directly (`check_headroom_c_free.py`): Point 87's own 6-point series (`n=127,509,1021,2039,4001,8009`) is monotonically INCREASING at every step, no dip. The skeptic's concern appears to reference a DIFFERENT, smaller, separate dataset — Point 64's own frozen 12-graph pilot at only `n=127/509/2039`, whose `1+CV²` values (`1.980/1.970/2.040`) DO dip at the middle point — a distinct artifact, per this project's own Gate 1 discipline (a verdict for one artifact does not transfer to another sharing a subject). Flagged and corrected, not silently ignored.

**(6) DROPPED entirely, not defended.** The first draft's aside comparing observed growth to a `log(log n))`-proportional model ("much closer than `log³n`") is withdrawn without argument — it re-opens, via an unexamined shortcut, the model-comparison question Point 87§4I already formally closed as near-uninformative over this `n`-range, and the skeptic's own (self-flagged `[ГИПОТЕЗА]`, not fully verified either) rough check suggests it may not even hold against this project's own established confidence intervals. Not worth re-litigating via a one-line ratio; Point 87§4K's own closure of the model-comparison line stands.

### Kill Analysis / Corrected Verdict

**What is killed:** the "novel Parseval connection" framing, and the specific absolute-number headroom claim (`6.3`, assuming a specific `C`).

**What survives, corrected and simplified:** `CV²(y*) ≤ n‖y*‖₂² − 1 ≤ C²log³n − 1` (unconditional, via `s≤n`, no empirical `n/s` claim needed) — this is genuine, if modest, progress: it gives a clean, honest, definitive answer ("yes, trivially, via already-established facts") to `activeContext.md`'s own long-flagged question, even though nearly all of the underlying pieces (the Parseval identity, the `O(log³n)` bound, its correct w.h.p.-to-expectation handling) were already present in this project's own prior work, just not explicitly chained together in `decision.md`'s own running text before this point. The `C`-free headroom ratio (`~6.4×` permitted vs. `~1.06×` observed) is real and survives: **current data cannot discriminate `(POL)` (CV² saturates) from a slow-but-unbounded alternative (e.g. `loglog n`-type growth) — both remain fully consistent with the known ceiling at these `n`.** `(POL)` itself remains **NOT PROVEN**; this point clarifies what is and is not already known about it, it does not advance a proof.

**Methodology lesson, same pattern as Point 91's, worth stating explicitly rather than letting it feel like a coincidence:** BOTH of Point 90's two "cheap, no computation" next steps, once actually attempted, turned out to contain a genuine, independently-confirmed error caught only by a real skeptic pass — one a test-design/numerical bug (Point 91), one an overclaimed novelty plus an avoidable methodological complication (this point). "Cheap and no computation" describes the COST of attempting a check, not the reliability of its first-draft output — the same discipline (context-asymmetric skeptic review before any positive claim reaches `decision.md`, independent re-verification of the skeptic's own claims before accepting them) was necessary here exactly as it was for every other point this entire window.

## Point 93 (2026-09-18) — `(POL)` proof-attempt line (Points 77-92) formally PARKED, per the user's own pre-agreed 5-step plan's own stated contingency

**Context.** Per the user's own explicitly-agreed autonomous plan from earlier this window ("го все по очереди автономно"): step 3 named the two cheapest untried next steps from Point 90; step 5 named the contingency — "if no new inequality found, PARK the project." Both named steps have now been attempted (Points 91, 92) and both were killed/corrected by real, independently-verified skeptic review, with no new proof-relevant inequality or technique surviving either. Per the pre-agreed plan's own trigger condition, this arc is now formally parked, not abandoned informally.

**Full Kill Analysis, revival condition, and scope boundary written up in `parked/H-CAT31-3-pol-proof-attempt.md`** (per FL's `null_results`/`parked` protocol), with a corresponding row added to `parked/INDEX.md`. Summary: 4 independent proof routes killed (Talagrand transfer, uncertainty-principle-on-supports, Young's/sup-norm, KKT/argmin-stability — Points 81/85/86/88); the `mean(CV²)` functional-form question formally closed as uninformative over the tested `n`-range (Point 87§4K, 3 converging lines of evidence); the Bandeira et al. proof-structure connection clarified but not exploited into new leverage (Points 90, 92, each independently corrected twice); both of Point 90's own final named next steps killed/corrected (Points 91, 92). `(POL)` itself remains an open mathematical question — not disproven, only every attempted approach to proving it has failed.

**What this does NOT park:** the broader `H-CAT31-3` experiment's other open questions (`F4rel`, `R_n`, composite `n`) and, most importantly, **this project's own ORIGINAL target, `Var(log(θ(G)/√n))=O(1/n)`, which is a different (and per an unverified `[WEAK]`-flagged external claim, possibly stronger-implying) statement than `(POL)`** — the `(POL)` detour began at Point 77 as a candidate route toward the original target, not a replacement for it, and its parking does not affect the original target's own status.
...
## Point 95 (2026-09-18) — `R_n` pre-registered follow-up (Point 53's own named next step): tightened reps, sent to a real skeptic BEFORE writing anything encouraging, and every positive-leaning conclusion drawn from the raw numbers was independently found unsupported — genuine, valuable new point estimates survive, but the scaling question `R_n` was meant to answer remains as open as before

**Context.** Per Point 53's own pre-registered "Next step" (cost-corrected: `4×` reps at `n=509/1021`, `2×` at `n=2039`), reused `check_first_chaos_decomposition_large_n.py`'s own functions (`sample_x_and_q`, `compute_decomposition`, `paired_bootstrap_ci`) UNCHANGED, read-only import, fresh non-overlapping seeds. Ran `n=509` (800 reps), `n=1021` (600 reps), `n=2039` (160 reps) — `1878s≈31min` total, matching the pre-registered cost estimate.

**Raw result:** `n·R_n`'s bootstrap 95% CI now clears zero at BOTH `n=509` (`0.653`, CI `[0.127,1.065]`) and `n=1021` (`0.721`, CI `[0.169,1.145]`) — it did not at either point in the original, noisier sweep. `λ_n` looks markedly more stable (`-2.974,-2.954,-2.949`, range `0.025`, vs. `~0.65` in the original sweep). An UNWEIGHTED OLS of `n·R_n` vs `n` across these 3 points gave slope `1.01×10⁻⁴`, `r²=0.988`. The exact small-`n` power-law prediction for `n·W_1` (`∝n^{0.239}`) now falls outside the observed CI at BOTH `n=1021` and `n=2039` (previously only at `n=2039`).

**Sent to a real, context-asymmetric skeptic BEFORE writing any of this up as encouraging** (per this window's own, by-now `[REPEAT×13]` pattern: every positive-leaning finding this entire window needed a real correction). **Verdict: every one of four draft conclusions was found unsupported, three explicitly `FALSIFIED` as stated.** All four re-derived independently here (`verify_skeptic_rn_review.py`) before accepting, not taken on the skeptic's word:

**(1) `[FALSIFIED, confirmed independently]` — `n·W_1` carries ZERO information beyond `λ_n`: it is the exact algebraic identity `n·W_1 = n·λ_n²/(2(n-1))`, verified to match the reported values to 4 decimal places at all three `n`.** "`λ_n` AND `n·W_1` are both now more stable" is the SAME observation counted twice, not two independent confirmations.

**(2) `[FALSIFIED, confirmed independently]` — the "power-law now fails at 2 points instead of 1" is an artifact of the extrapolated prediction carrying NO uncertainty band, not a real rejection of the exponent `0.239`.** A pairwise test comparing the OBSERVED `509→1021` change in `n·W_1` against the change the power law PREDICTS gives `z=-1.20` (`p=0.23`) — not significant. A properly weighted fit of the exponent using only the 3 new points gives `a=-0.016±0.153`, 95% CI `[-0.316,0.284]` — **`0.239` sits comfortably INSIDE this interval.** Since `n·W_1≡n·λ_n²/(2(n-1))` (finding 1) and `λ_n` is nearly flat across these 3 points, the observed curve is close to constant while the small-`n`-calibrated power law's prediction grows — meaning how many of the 3 points "fail" to contain the prediction is controlled entirely by CI width, i.e. by REP COUNT, not by anything about the true scaling. Confirmed directly: `n=2039`'s CI is the widest (fewest reps) and happens to still contain its own prediction; `n=1021`'s is narrower and does not — a rep-count artifact, not new scaling information.

**(3) `[FALSIFIED, confirmed independently]` — the `n·R_n` "trend" (`r²=0.988`) is a well-behaved CONSTANT once the three points' own uncertainties are used instead of treating three noisy estimates as exact.** A weighted `χ²`-test for `n·R_n=`constant across the 3 points gives `χ²=0.097` on 2 degrees of freedom, `p=0.95` — the flat model fits essentially perfectly. The unweighted OLS `r²=0.988` that looked like a real trend is exactly the kind of artifact this project's own discipline elsewhere warns against (treating noisy point estimates as if they carried no uncertainty). **A weighted fit of `n·R_n∝n^b` gives `b=0.153±0.479` (95% CI by `z`; wider still by the more honest `t`-distribution with 1 df) — this interval contains BOTH `b=0` (a true plateau, consistent with `Var(X_n)=O(1/n)`) AND `b=1` (`R_n=Θ(1)`, meaning `Var(X_n)` would NOT decay like `1/n` at all).** The 3 tightened points, even with 4-8× the original reps, cannot distinguish these two qualitatively opposite possibilities — this is the single most important, sobering finding of this follow-up: it does not merely fail to confirm the target hypothesis, it fails to meaningfully constrain it at all on the scaling question specifically (the one thing Point 53's own "next step" was designed to test).

**(4) `[WEAKENED, correct but trivial]` — `n·R_n`'s CI clearing zero at `509`/`1021` is real (independently confirmed: combining the OLD and NEW draws at each `n` via inverse-variance weighting, checking for seed-batch inconsistency first — old vs. new agree closely, `z=0.02` at `n=509`, `z=0.33` at `n=1021`, no evidence of a lucky/cherry-picked seed batch — gives combined `z=2.84`/`2.73` for `R_n>0`, barely surviving a 3-comparison Bonferroni threshold of `2.39`), but this answers a question that was never actually open: `R_n≥0` ALWAYS, exactly, by construction (`R_n` is a sum of squared Fourier-Walsh coefficients, Point 53's own already-established decomposition) — "confirmed real, not just noise centered at zero" is a true statement about the ESTIMATOR finally having enough power to see a known-nonnegative quantity clearly, not an empirical discovery about `R_n` itself. The only question that was ever open — how `R_n` SCALES with `n` — is untouched by this finding, per (3) above.

**Two additional findings, independently confirmed, worth recording:** (a) explicit alternative curves for `n·W_1` — a genuinely UNBOUNDED-growth form (`3.244·n^{0.05}`, giving `n·W_1→∞`, i.e. no plateau ever) and a slowly-DECREASING form (`4.791-0.0579·ln n`) — both fit inside all three observed CIs exactly as well as a "plateau" reading does; "plateau" was a narrative choice, not a conclusion the data forces. (b) a legitimate estimator-bias concern: `Ŵ_1=λ̂²/(4m)` is upward-biased by construction (`E[λ̂²]=λ²+Var(λ̂)`), correspondingly biasing `R̂_n` downward, by an amount that scales as `~1/N_reps` — and since rep counts differ substantially across the 3 tested `n` (800/600/160), this bias is itself confounded with `n`, further undermining any raw trend read from these specific numbers.

### Kill Analysis

**What is killed:** none of the four specific "encouraging" conclusions drafted from the raw numbers survives as originally stated — three were actively wrong (not just weak), one was true but answered an already-settled question.

**What is NOT killed:** the underlying `Var(X)=W_1+R_n` decomposition itself (an exact identity, untouched); the genuinely sharper new point estimates for `λ_n≈-2.95` and `n·R_n≈0.70` (weighted mean across the 3 tightened points, `χ²=0.097` supporting a single shared value); the general open question of whether `R_n` (hence `Var(X_n)`) scales as `O(1/n)` — still completely open, `[NEEDS-REAL-DATA]` in the skeptic's own vocabulary, not resolved in either direction by this follow-up.

**Revival condition / concrete next step, per the skeptic's own fallback recommendations, cheapest first:**
1. **Free, already available:** combine the OLD (200/150/80-rep) and NEW (800/600/160-rep) draws via inverse-variance weighting at each `n` — already done above for the `R_n>0` question at `509`/`1021` (giving `z=2.84`/`2.73`); the SAME combination should be applied to the scaling-exponent question too before spending any new compute.
2. **Cheap, ~20 additional minutes:** equalize rep counts across all three `n` (e.g. `800` at all three, not tapering to `160` at `n=2039`) — removes both the bias-confound named above and the "number of power-law 'failures' depends on rep count" artifact; `n=2039` currently carries only `~17%` of the statistical weight of `n=509` in a variance-weighted fit.
3. **Before any further compute:** reformulate the target explicitly as "what is the `95%` CI on the scaling exponent `b` in `n·R_n∝n^b`," with a PRE-REGISTERED stopping rule stated in terms of how many points/reps would be needed to exclude `b=1` (the "no `O(1/n)` decay at all" scenario) — not "is `R_n` positive," which this point's own finding (3) shows is not actually informative.
4. Replace the percentile bootstrap for `n·R_n` specifically with a BCa or studentized method — the skeptic noted the observed CIs are visibly skewed and `n·R_n` sits near a boundary (`0`), exactly the regime where percentile-bootstrap coverage is known to be weakest.

**Methodology lesson, same `[REPEAT]` pattern as every other point this window, now `[×13]`:** dispatching this to a real skeptic BEFORE writing an encouraging draft into `decision.md` — rather than after — meant zero overclaiming reached the permanent record. The specific NEW lesson here, not previously named this precisely in this project: an unweighted OLS "trend" fit computed from a handful of noisy point estimates, without using those estimates' own uncertainties as weights, is a generically unreliable way to detect a real trend — it can manufacture an apparently clean `r²` from what a proper weighted `χ²`-test shows is statistically indistinguishable from a flat constant.

## Point 96 (2026-09-18) — atomization of the headline claim `C0: V_n=O(1/n)` into 5 rival/route sub-claims (C1-C5), relayed and cross-checked against this project's own history BEFORE any new work: 3 of 5 (`λ_n`/`R_n`) are the just-parked Point 95 question restated, not new; C4 (`K_n`) has an unacknowledged 20+-point history (Points 56-78) already reduced to "empirical channel EXHAUSTED"; the one genuinely fresh route (C5, mean-gap) gives a real, load-bearing finding checked here

**Context.** User relayed an external atomization of the project's headline claim into 5 independently-attackable sub-claims: `C1: |λ_n|=O(1)`, `C2: R_n=O(1/n)` (jointly `⟹C0`), `C3: sup_n|λ_n|=∞` (`⟹¬C0`, the rival to C1), `C4: K_n:=n²E[δ_i²]=O(1)` (self-sufficient route, via Efron-Stein), `C5: g_n:=E[θ(G_n)]-√n=O(n^{-1/2})` (self-sufficient route, via Point 94's own inequality). Instructed to cross-check against this project's own history before proceeding, per the Adaptive Iteration Branch Rule — not to accept the relayed atomization at face value.

**Cross-check result, reported to the user BEFORE any new computation: the relayed atomization significantly understates how much of this territory is already-explored.**
- **C1/C2/C3 are literally the `λ_n`/`R_n` scaling question PARKED earlier THIS SAME SESSION (Point 93, extended Point 95)** — the exact `V_n=λ_n²/(4m)+R_n` decomposition, with a fully-specified, already-agreed revival condition (Point 95's own 4-step plan) never yet executed.
- **C4 (`K_n=O(1)`) is NOT a fresh one-shot claim.** Points 56-78 (22 points, not summarized in the relayed message) measured this exact object at length: `K_n`'s own 4-point weighted-power-law slope (`n=127/509/1021/2039`, `500` reps each) was `b=0.0671±0.0516`, `95%` CI `[-0.034,0.168]`, and Point 77 formally declared the empirical channel **EXHAUSTED** ("Explicitly NOT a revival condition: 'measure one more `n`'"), reducing the target to a joint-moment quantity `J_n=O(1)`/MIX4 (Point 78's own correction) — the SAME underlying joint-moment target the later `(POL)` proof-attempt line (Points 79-92, PARKED Point 93) pursued via its marginal (not joint) form. `K_n=O(1)` is proven-SUFFICIENT (Efron-Stein) but explicitly NOT proven-necessary (Point 78's own correction to Point 77's overclaimed `⟺`).
- **C5 (`g_n=O(n^{-1/2})`) is the one genuinely untested route** — Point 94 established the exact inequality `V_n≤2g_n/√n` (`⟹` in the right direction for C5 to work), but `g_n`'s own scaling was never numerically checked. Pursued below.

**User's decision, per the same cross-check:** (1) execute Point 95's own named revival plan for C1/C2/C3 (combine old+new draws, equalize reps to `800` at `n=509,1021,2039`, refit the scaling exponent `b` in `n·R_n∝n^b` — separately reported, see below/next point); (2) for C4, run ONLY the one concretely-named, not-yet-done cheap check from this project's own history (Point 76 explicitly flagged it as skipped for `K_n`: the `n=127`-exclusion leverage-robustness check already run for the closely related `J_n`) — not a fresh proof attempt, given the exhausted-channel status; (3) for C5, a genuinely new numerical test.

### C4 (`K_n`) — `n=127`-exclusion robustness check, extended to full leave-one-out after a real skeptic correction

Reused `ppl_gate_pilot.py`'s own `weighted_power_law_fit` function UNCHANGED and the already-stored `K_n`/`K_n_SE` raw values in `metrics/ppl_gate_pilot.json` (no new LP computation). First draft (excluding only `n=127`, matching Point 76's own missing check): `b=0.0941±0.1056`, `95%` CI `[-0.113,0.301]`, `p=0.373` (vs `p=0.193` at 4 points) — drafted as "robust to excluding `n=127`, unlike `J_n`."

**Sent to a real, context-asymmetric skeptic BEFORE writing this up as a finding.** Verdict: the "robust" framing is an **overclaim**, for reasons independently re-verified here before accepting (`kn_loo_verify.py`):
1. **Full leave-one-out (all 4 possible exclusions, not just `n=127`)**: `b=0.0941` (excl `127`), `0.0651` (excl `509`), `0.0506` (excl `1021`), `0.0847` (excl `2039`) — **every single exclusion gives a positive point estimate**, none reaching significance, but the "robust to `n=127`" framing hid that excluding `n=2039` (the most asymptotically relevant point) ALSO raises `b` (to `0.0847`), not just `n=127`.
2. **Hat-leverage `h_ii` for the full 4-point fit**: `0.823/0.305/0.320/0.552` for `n=127/509/1021/2039` (sum `=2`, matching the 2 free parameters exactly) — confirms BOTH endpoints (`n=127` and `n=2039`) carry above-average influence (`h̄=0.5`), not just the smallest `n`.
3. **The "still not significant" result was foreordained, not informative — corrected from the skeptic's own arithmetic error, independently caught here.** The skeptic's report claimed a `78%` growth in `K_n` over `509→2039` was needed to reach significance at 3 points; independently recomputed, the correct figure is a **`33%`** growth (`(2039/509)^{0.2069}=1.333`, not `1.78` — the skeptic's own `"e^{0.207·2.78}"` used an incorrect exponent, `2.78` instead of the correct `ln(2039/509)=1.388`; re-verified directly, not accepted on the skeptic's word, per this session's standing rule). The corrected qualitative conclusion is unchanged (observed growth was only `12.3%` (`ratio 1.123`), well below the `33%` needed) — the test genuinely lacked power to detect the actual small observed change — but the specific severity is less extreme than the skeptic's own (erroneous) `78%` figure suggested.
4. **No bug in the core arithmetic** (all leave-one-out values, hat-leverages, and the "doubling reps at all 4 points" SE projection `0.0365` independently re-verified to match); but the original "sanity check" (reproducing the already-stored fit from its own stored inputs) was a tautology, not a real validation control, and the CI's own weighting scheme (weight `∝1/SE²`, correlated with the point estimate itself) is a secondary, unaddressed concern.
5. **Differentiation/kill-power near zero, per this project's own Cheapest Differentiating Test Protocol**: `K_n=O(1)` is sufficient-not-necessary for `C0`, so even a clean negative result here would not kill the headline theorem — this specific check was never going to be a high-value expenditure, independent of its outcome.

**Corrected finding, replacing the drafted "robust" conclusion:** leave-one-out gives a small, consistently POSITIVE point estimate (`b∈[0.051,0.094]`) across all 4 possible exclusions, never individually significant, but the current data (`500` reps at 4 points spanning `n=127..2039`, `1.15` decades) lacks the power to distinguish `b=0` from `b≈0.1-0.2` — a range that would already break the `K_n=O(1)` target this route needs. **Uninformative, not robust.** A single additional point at `n≈8191` (same relative SE `~10%`) would sharpen `SE_b` to `≈0.033` — MORE informative than doubling reps at all 4 existing points (`SE_b≈0.037`) — independently verified, not accepted from the skeptic's own number. Not pursued in this point (matches the project's own Point 77 budget discipline: no compute spent on this specific measurement channel without a new, separately pre-registered justification).

**Kill Analysis.** What is killed: the drafted "robust, no leverage artifact" characterization of `K_n`'s own scaling-exponent measurement. What is NOT killed: `K_n=O(1)` itself (still open, exactly as open as before this check — this check never had the power to move it either direction); the broader `C4` route's status as proven-sufficient-not-necessary and empirical-channel-exhausted (Point 77/78, unchanged). **Revival condition:** one additional Monte Carlo point at `n≈8191` (feasibility of the LP solve at this `n` not yet confirmed) — matches this route's own already-named, not-yet-executed next step, not a new one invented here.

**Artifacts:** `kn_leverage_check.py`, `kn_leverage_check_result.json`, `kn_loo_verify.py` (independent re-verification, not committed as a separate script — scratchpad).

## Point 97 (2026-09-18) — C5 (`g_n=E[θ(G_n)]-√n=O(n^{-1/2})`): the drafted "evidence against C5" conclusion is FALSIFIED at its root — `g_n`'s own scaling is algebraically near-identical to `V_n`'s ALREADY-measured, ALREADY-`REJECTED`-for-exact-`-1` exponent, and the data used isn't even prime `n`. Real, substantive skeptic catch, independently re-verified in full before writing this.

**Context.** Per Point 96's plan: test whether `g_n` decays at rate `n^{-1/2}`, using Point 94's own established inequality `V_n≤2g_n/√n` and this project's existing 9-point sweep (`metrics/run.json`, `n=32..3000`, H-CAT31-1's own original data). First-draft result (ad hoc exclusion of 3 low-power points where `g_n` was within noise of zero): weighted fit `b=-0.0871±0.1337`, `95%` CI `[-0.349,0.175]` — target `-0.5` sits `3.09σ` OUTSIDE this CI, drafted as "evidence AGAINST the mean-gap route, not merely inconclusive."

**Sent to a real, context-asymmetric skeptic BEFORE writing this up.** Verdict: **FALSIFIED**, on two independent, load-bearing grounds — both independently re-derived from scratch here before accepting (`c5_skeptic_verify.py`, `c5_debiased_refit.py`, `c5_debiased_refit2.py`):

**KILL #1 — this is not an independent test at all.** By the exact Taylor expansion of `cosh` already used to derive Point 94's own inequality: `g_n/√n = E[cosh(X_n)]-1 = V_n/2 + E[X_n⁴]/24 + ... ≥ V_n/2`. This means `g_n`'s own scaling exponent is algebraically forced to `b_g ≈ 1/2 + b_V` (`b_V` = `V_n`'s own exponent), NOT an independent quantity. **Independently re-verified, numerically, to high precision:** a properly DEBIASED estimator `ĝ_n := √n·(mean(θ/√n) - 1 - mean(X_n))` (subtracting the pure-noise, zero-mean linear term `mean(X_n)` — exact `E[X_n]=0` by the same self-complementarity argument — removes the dominant sampling-noise component, exactly the same variance-reduction trick this project's own `R_n` cross-fit decomposition already uses) gives a MUCH tighter fit: `b_g=-0.4260±0.0245` (all 9 points, no ad hoc exclusion, `χ²=7.81`, `dof=7`, clean fit) — and `V_n`'s own ALREADY-STORED exponent in this SAME file (`metrics/run.json`'s `fit.weighted_ols`) is `b_V=-0.9126±0.0264`. Predicted `0.5+b_V=-0.4126` vs. independently-fit `b_g=-0.4260`: difference `0.0134`, well within the combined SE (`0.036`) — **confirms the near-identity to high precision, not a coincidence.**

**KILL #2 — the "evidence against C5" framing is circular: this SAME file already records the underlying finding as `REJECTED`.** `metrics/run.json`'s own top-level `"verdict": "REJECTED"` (for `V_n`'s exact `-1` exponent, `95%` CI `[-0.975,-0.850]`, excluding `-1`) is the SAME underlying fact this "new" C5 test rediscovered, via a noisier derived statistic (`g_n`'s own SE is `4-33×` larger than `V_n`'s own direct estimator's SE, per the skeptic's own antithetic-estimator calculation, independently plausible from the `rel_se` values directly in the file — `8-23%` for `g_n` vs `V_n`'s own tighter `2.6%` relative SE on the exponent itself).

**Additional, independently-confirmed problem, found by the skeptic and verified here: NONE of the 9 tested `n` values are prime.** `sympy.isprime` check: `32,64,128,256,512,1024,1536,2048,3000` — all composite (mostly powers of `2`). This project's headline claim `C0` and its entire `W_1/R_n` point-4-symmetry machinery (`λ_n`, `K_n`, `J_n`, the exact `s=2Q+1` identity) is specifically about PRIME `n` (the multiplicative group `Z_n^×` acting transitively on generator coordinates) — Point 94's own `cosh`-bound inequality likely holds regardless of primality (a general vertex-transitive/self-complementary argument), but this specific 9-point sweep's OWN scaling behavior is not established to be representative of the prime-`n` regime `C0` is actually about — the same prime-vs-composite distinction this project's own separate `H-CAT31-2` branch exists to investigate (and found REJECTED as a confound with `n` itself, per that experiment's own 2026-09-18 continuation).

**Ad hoc exclusion criticized independently and correctly.** The first draft's exclusion of `n=1024,2048,3000` (the 3 LARGEST tested `n`, where `g_n` was statistically consistent with 0) was a selection-on-the-outcome error: those are exactly the points where a genuine `n^{-1/2}` decay would be EXPECTED to have pushed `g_n` toward the noise floor — excluding them mechanically biases the fitted exponent toward 0 (flat), regardless of the true rate. The debiased, all-9-points fit above avoids this entirely.

**Independently re-checked, one thing the skeptic reported with a computational slip elsewhere caught in a DIFFERENT check this session (K_n, Point 96) — re-verified this skeptic's own numbers carried NO equivalent error here**: leave-one-out on the debiased 9-point fit gives `z`-distance from `-0.5` ranging `2.13` (excluding `n=1536`) to `3.46` (excluding `n=2048`) — mostly still excludes `-0.5`, but the minimum (`2.13`, excluding `n=1536`) falls BELOW the correct `t`-critical value for `8` points/`6` residual dof (`t≈2.45`), meaning the "excludes `-0.5`" finding is not uniformly robust to every possible single-point exclusion, though it holds under most of them.

**Kill Analysis.** What is killed: the drafted "C5 gives evidence AGAINST the mean-gap route" framing, and more importantly the premise that empirically measuring `g_n` from THIS dataset provides ANY evidence independent of `V_n`'s own already-recorded exponent measurement. What is NOT killed: the mean-gap route as a THEORETICAL proof strategy — Point 94's own inequality (`V_n≤2g_n/√n`) is untouched and exact; `C5` remains a live PATH to `C0` in the sense that an INDEPENDENT, non-empirical bound on `E[θ]` (e.g. via LP duality, not Monte Carlo re-estimation of the same quantity) would still work if found — the empirical channel specifically is what fails to add information, not the mathematical strategy.

**What this does NOT mean.** Does NOT mean `C0` is false — `V_n~n^{-0.91}` (already `REJECTED` for exact `-1`, already known, `BENCHMARK-SPECIFIC-NUMERIC-RESULT` per the 2026-09-10 novelty audit) is a real but modest deviation from the target rate, not evidence of no decay; the skeptic's own alternative-model check (`g∝√(log n)/n` fits the same data with local slope `-0.404`, statistically indistinguishable from the pure power-law `-0.5` fit at this range) shows a log-correction is equally consistent, not ruled out. Does NOT mean the `K_n`/`J_n` routes (Point 96) are affected — unrelated objects. Does NOT mean this specific dataset is untrustworthy for what it WAS already used for (the cosh-bound sanity check, Point 94, which only needed the inequality direction, not a scaling exponent).

**Revival condition, named by the skeptic, not invented here:** (a) a genuinely independent, non-empirical upper bound on `E[θ(G_n)]` via LP duality or a union-bound argument, not re-estimated from Monte Carlo data already used to measure `V_n` itself; (b) if pursuing the empirical channel further despite (a)'s absence, use prime `n` specifically (matching `C0`'s own scope) and a variance-reduced antithetic estimator (`(r+1/r)/2`, exact via the Lovász identity `θ(G)θ(Ḡ)=n`, zero new LP solves) — projected `4-33×` SE reduction over the naive estimator, not yet implemented. Neither pursued in this point, matching the same discipline as Point 96/the broader session: no manufactured further computation without a concretely named, independently-justified next step.

**Artifacts:** `c5_mean_gap_test.py` (first draft, flawed exclusion criterion — kept for the record, not deleted), `metrics/c5_mean_gap_result.json`, `c5_skeptic_verify.py`/`c5_debiased_refit.py`/`c5_debiased_refit2.py` (independent re-verification, scratchpad, not committed separately).

**Methodology lesson, `[REPEAT]`-worthy, sharper than the usual pattern this window.** Every prior skeptic catch this window found an ERROR in a draft's REASONING about real, independent data. This one is different and more fundamental: the test itself was measuring the SAME quantity as an already-completed, already-recorded measurement, via a strictly noisier derived statistic — the "new evidence" was never capable of being new information, regardless of how carefully the statistics were done. **Before designing a new empirical test for a proof-route sub-claim, check whether the claim's own DEFINING inequality algebraically reduces it to an already-measured quantity** — this would have been caught for free by writing out the Taylor expansion of `cosh` (already sitting in this project's own Point 94 derivation) BEFORE writing any Monte Carlo code, not after.

## Point 98 (2026-09-18) — C1/C2/C3 (`λ_n`/`R_n` scaling): executed Point 95's own named revival plan in full (combined old+new draws, equalized to `800-1000` reps at `n=509,1021,2039`, BCa bootstrap, nested-bootstrap exponent CI) — first draft looked like a genuinely new, concerning result; skeptic found the draft rested on a methodological error (delta-method log-SE instead of the already-computed equivariant BCa interval); corrected, `R_n`'s own scaling remains `STILL INCONCLUSIVE` exactly as Point 95 left it — but the SAME pooled data, used to measure `Var(X_n)` DIRECTLY instead of through the `R_n` decomposition, gives the single most precise, most informative, most positive number for `C0` this entire arc has produced

**Context.** Per Point 96's cross-check and the user's explicit instruction, executed Point 95's own 4-step revival plan for the `λ_n`/`R_n` scaling question (the PARKED material, C1/C2/C3 in the relayed atomization) in full: (1) combined OLD (`200/150/80`-rep, `RNG_SEED_BASE`) and NEW (`800/600/160`-rep, Point 95's own fresh seed offset) draws by pooling raw `(X,Q)` pairs (equivalent to inverse-variance weighting for i.i.d. equal-per-observation-variance data — a stronger combination than weighting point estimates, since it re-derives the decomposition from the full pooled sample); (2) equalized rep counts to `800` at all three `n` (n=1021 extended `600→800`, `n=2039` extended `160→800`, `~9542s`/`159min` of LP-solve time at `n=2039` alone — the dominant cost of this whole revival); (3) reformulated as a `95%` CI on the scaling exponent `b` in `n·R_n∝n^b`; (4) used a BCa bootstrap (bias-corrected, accelerated, `5000` reps per `n`) for `n·R_n` itself, plus a NESTED bootstrap (`2000` outer iterations, resampling all 3 pooled datasets jointly per iteration, refitting `b` each time) for `b`'s own CI — replacing the earlier normal-approximation entirely, exactly as named.

**First-draft result:** point estimate `b=0.4779` (weighted fit using BCa-CI-derived SE as weights), nested-bootstrap `95%` CI `[0.044, 0.943]` — technically excluding BOTH `0` and `1`, a qualitatively new outcome not anticipated by the pre-registered three-bucket rule (LEAD/CONCERNING/STILL INCONCLUSIVE), drafted as "a genuinely concerning finding against the project's main conjecture, stronger than any prior measurement in this project's history."

**Sent to a real, context-asymmetric skeptic BEFORE writing this up.** Verdict: **FALSIFIED**, on three independent grounds, every one independently re-derived from scratch here before accepting (`rn_skeptic_verify.py`) — matches the skeptic's own numbers to 3-4 decimal places in every case:

**KILL #1 — measuring `Var(X_n)` DIRECTLY on this SAME pooled data gives a far more precise, genuinely POSITIVE result for `C0`, and makes the drafted "R_n eventually dominates" extrapolation self-contradictory.** `n·Var(X_n) = n·W_1+n·R_n` computed directly: `5.161/4.989/4.871` at `n=509/1021/2039` — a weighted fit (equivariant BCa log-SE) gives `b_Var=-0.042±0.046`, `95%` CI `[-0.132,0.048]` — **independently re-verified here to match the skeptic's own `-0.042±0.047` exactly.** This is `5×` more precise than the `R_n`-only exponent (`SE 0.046` vs `0.27`) and is consistent with (in fact, mildly supports) `Var(X_n)=O(1/n)` directly — the single most informative number this entire multi-hour revival effort produced, and it was never the original target of the analysis. Separately: since `R_n≤Var(X_n)` always (both nonnegative, `R_n` is one component of the sum), the drafted power-law extrapolation for `R_n` (`b≈0.48`) is internally inconsistent with the much tighter near-flat `Var(X_n)` fit — extrapolated forward, the fitted `R_n` power law would eventually EXCEED `Var(X_n)` itself (a mathematical impossibility) at `n≈36,000`, confirming the `R_n`-alone extrapolation cannot be taken at face value beyond the tested range.

**KILL #2 — the "excludes both 0 and 1" finding rested on a methodological bug: delta-method log-SE instead of the already-available, correctly-equivariant BCa interval.** The first draft used `se_log = SE(Y)/Y` (delta-method approximation) as the WLS weight for the exponent fit — but the BCa bootstrap CI is already transformation-equivariant (Efron 1987): `ln(BCa CI for Y)` IS the correct BCa CI for `ln Y`, directly, no delta-method approximation needed. Given the visibly right-skewed `n·R_n` bootstrap distributions (long lower tails at all three `n`), the delta method UNDERESTIMATES the log-scale spread by `6-29%` across the three points. **Recomputed with the correct, equivariant log-SE: `b=0.4869±0.2680`, `95%` CI `[-0.038,1.012]`** — independently re-verified here to match the skeptic's `0.4868±0.2680` to 4 decimal places — **the corrected CI contains BOTH `0` and `1` again.** The finding collapses back to exactly the third pre-registered bucket: `STILL INCONCLUSIVE`.

**KILL #3 — the (already-collapsed) finding does not survive leave-one-out either.** Independently re-verified: excluding `n=509` gives `b=0.641±0.462`, CI `[-0.266,1.547]`; excluding `n=1021` gives `b=0.429±0.303`, CI `[-0.165,1.023]`; excluding `n=2039` gives `b=0.219±0.710`, CI `[-1.172,1.610]` — **every single leave-one-out CI contains both `0` and `1`.** The corrected point estimate (`b≈0.49`) sits almost exactly midway between the two competing scenarios it was meant to discriminate (`0.478` from `0`, `0.522` from `1`) — the textbook signature of underpowered data landing between two hypotheses, not a genuine third outcome.

**Corrected finding, replacing the drafted "concerning" conclusion:** `R_n`'s own scaling exponent remains `b=0.49±0.27`, `95%` CI `[-0.04,1.01]` — **STILL INCONCLUSIVE**, exactly Point 95's own bucket, even after a `5×` increase in effective sample size (`SE` narrowed from `0.479` to `0.268`, matching the `√5≈2.2×` reduction pure rep-count scaling predicts — no more, no less; the additional data bought precision, not resolution, because the underlying signal-to-noise ratio at this `n`-range was never large enough to separate `b=0` from `b=1` with only 3 points spanning `1.39` natural-log units). **The genuinely new, positive, well-supported result of this whole effort is the DIRECT `Var(X_n)` fit** (`b_Var=-0.042±0.046`), not the `R_n`-decomposed one — worth recording as the headline number from this point, not the R_n exponent the revival plan originally targeted.

**Kill Analysis.** What is killed: the drafted "excludes both 0 and 1, concerning finding" characterization of the `R_n` exponent — a real methodological artifact (delta-method vs. equivariant log-SE), not a genuine result. What is NOT killed: `R_n`'s own scaling remains genuinely open, exactly as before this revival (matching Point 95's own honest assessment — more data did not resolve it, only sharpened the existing ambiguity). **What is newly, positively supported:** `Var(X_n)` itself, measured directly (bypassing the `R_n` decomposition entirely) at these 3 prime `n`, is consistent with `O(1/n)` with real, meaningful precision (`b_Var` 95% CI excludes anything much beyond `±0.13`) — the first genuinely tight, genuinely positive confirmatory number for the headline theorem `C0` produced by any experiment in the `C1`-`C5` atomization this session.

**Revival condition for `R_n`'s own scaling question specifically, named by the skeptic, independently endorsed:** a SINGLE additional point at `n≈8191` (next convenient prime in range, matching this project's own established `n≈2×` spacing) would roughly double the log-`n` leverage (from `1.39` to `2.78` natural-log units) — more informative than any further rep-count increase at the existing 3 points. Explicit kill criterion for a future attempt, stated in advance: predicted `n·R_n` under the pure power-law (`b≈0.5`) scenario vs. the log-saturating alternative (`n·R_n→` a constant, consistent with the direct-`Var_n` finding above) diverge by `~3.5σ` at `n=8191` given comparable rep counts — a genuinely differentiating test, unlike anything in this `n`-range so far. NOT pursued in this point (would require a fresh, separately-costed LP-solve budget at a new `n`, matching this project's own discipline against uncosted compute expansion).

**Batch-consistency check (same style as Point 95's own R_n>0 check), run and confirmed clean:** regenerated the OLD batch alone at each `n` and compared against the ALREADY-RECORDED Point 95 NEW-only values (independent, non-overlapping seed regions): `z(new-old)=-0.338/-0.147/-0.566` at `n=509/1021/2039` — no evidence of a batch effect at any `n`, confirming the pooling in this point is combining genuinely consistent, not spuriously-differing, independent samples.

**Artifacts:** `rn_revival_combined.py` (pooling/equalization), `rn_revival_analysis.py` (first-draft BCa+nested-bootstrap fit, delta-method bug), `rn_skeptic_verify.py` (independent re-verification with the corrected equivariant log-SE, the direct-`Var_n` fit, and leave-one-out), `rn_revival_batch_check2.py` (old-vs-new batch consistency), `metrics/rn_revival_pooled_raw.json`, `metrics/rn_revival_analysis_result.json`, `metrics/rn_skeptic_verify_result.json`.

**Methodology lesson, `[REPEAT]`-worthy, a THIRD distinct flavor this window (after Point 97's algebraic-redundancy catch and the more familiar overclaimed-significance pattern): a bootstrap CI's own transformation-equivariance property (BCa specifically) is easy to silently discard by reflexively reapplying a delta-method approximation on top of an already-correct interval** — the fix was not "collect more data" or "use a fancier bootstrap," it was recognizing that a correctly-computed interval was already sitting there, and a second, unnecessary approximation was quietly degrading it. Worth a standing check for future exponent fits in this project: when a BCa (or any transformation-equivariant) CI is already available for the untransformed quantity, use its own log directly — never re-derive an approximate log-SE from it.

## Point 99 (2026-09-18) — Epistemic-status correction for Points 96-98: downgrade "independent skeptic" language, tighten the direct-`Var_n` claim's own scope, freeze the C1-C5 phase's outcome as one status line

**Trigger.** An external methodological read-through of Points 96-98 (relayed by the user) raised two load-bearing corrections not yet made explicit in this file, plus a state-tracking correction about this session's own git history. All three independently checked against this project's own standing rules before accepting — none taken on the reviewer's word alone, matching this session's own discipline.

**Correction 1 — "independent skeptic" overstates the verification strength actually available here.** `rules/falsification-ladder.md`'s own Independent Verification Strength Ladder ranks "same model, isolated context" as **Weak-Medium**, distinct from "different model" (Medium) or "independently-written code" (Strong). This project's own `pearl_registry/INDEX.md` (2026-07-19 row, RDR 2.1 "Reproducer" pearl) already documents, as settled fact, that **no cross-model invocation channel is wired in this repo** — every `Agent(skeptic, ...)` dispatch this window (Points 96/97/98) ran the same underlying model in an isolated context, not a genuinely different verifier. The Context Asymmetry Rule (claim.md/code only, no reasoning chain) is real and was followed correctly — it buys protection from agreeableness bias within a single dispatch, which is exactly why it caught 3/3 real errors this window — but it is not the same property as cross-model or cross-implementation independence. **Corrected epistemic status for all three Points 96/97/98 skeptic verdicts, and for this session's own re-derivations of the skeptic's numbers: `INTERNALLY CROSS-CHECKED` (same-model, context-asymmetric, Weak-Medium on the project's own Ladder), not `INDEPENDENTLY_VERIFIED`.** This does not change any of the three corrected findings — the errors caught (K_n overclaim, C5 algebraic redundancy, R_n delta-method bug) are real, independently re-derivable from the raw data by anyone, and were in fact independently re-derived here with fresh code before being accepted. It changes only how much additional confidence the word "skeptic" should be read as buying beyond what the fresh re-derivation itself already provides — registry/decision language should not let "context-asymmetric skeptic" imply "cross-model verified" retroactively.

**Correction 2 — the direct `n·Var(X_n)` fit (`b_Var=-0.042±0.046`, Point 98 KILL #1) is supporting empirical evidence for `C0`, not evidence toward proving the asymptotic rate.** Restated per this project's own EstimandOps "what this does NOT mean" discipline (already applied to Point 97, omitted for this finding at the time it was written): a 3-point log-log fit with a 95% CI of `[-0.13,0.05]` is consistent with `O(1/n)` and is the tightest, most positive number produced by this arc — but three points spanning `n=509..2039` (`1.39` natural-log units) cannot themselves establish an asymptotic rate; the same data is equally consistent with a slowly-varying non-power-law form that happens to look near-flat-in-log over this narrow range (the same caveat Point 97 itself raised for `g_n`'s own log-correction alternative). **Corrected framing: `b_Var` is the strongest available empirical data point consistent with `C0`, not confirmatory evidence for the asymptotic claim itself** — the distinction matters because the former is what Point 98 actually measured and the latter is a stronger claim no finite-`n` Monte Carlo experiment in this project can make.

**Correction 3 — freeze the C1-C5 phase's outcome as one project-status line, and record the intermediate state honestly:**

> **C1/C2/C3 revival: INCONCLUSIVE → PARKED** (Point 95's own revival condition executed in full at Point 98; result non-discriminating, `R_n`'s own exponent CI `[-0.04,1.01]` contains both target scenarios).

> **Main claim `C0` OPEN. Point-95 revival executed and non-discriminating. C1-C3 re-PARKED; C4 claim OPEN / empirical channel exhausted (Point 96); C5 claim OPEN / tested numerical route invalid as a discriminator (Point 97). No new active numerical branch justified by this phase alone.**

This does not add a new verdict — it restates Points 96/98's own already-written Kill Analyses (`K_n=O(1)` "still open, exactly as open as before this check"; `R_n`'s own scaling "STILL INCONCLUSIVE... exactly Point 95's own bucket") as a single scannable line, matching this project's own Checkpoint Fidelity discipline (`memory-protocol.md`: state should be reconstructable without re-reading the full transcript).

**Correction 4 (state-tracking, not epistemic) — local merge is not the same state as pushed/published.** As of this point, Points 96-99's own git history (2 commits + 1 merge commit) exists on this machine's local `main` only. Per this project's own standing discipline (`git push` requires a live, per-instance `AskUserQuestion` confirmation, never batched, never assumed from a prior approval), nothing in this arc should be described as `PUBLISHED`/`REMOTE-SYNCED` until that confirmation is obtained and the push actually executes. Correct state as of this point: **`REGISTERED-LOCAL`.**

**Kill Analysis.** What is killed: any reading of Points 96-98's "skeptic"-labeled corrections as cross-model-independent verification, and any reading of Point 98's `b_Var` finding as evidence for the asymptotic rate rather than for the current finite-`n` data being consistent with it. What is NOT killed: the three corrected findings themselves (K_n overclaim, C5 redundancy, R_n delta-method bug) — all three remain correct, re-derivable, and load-bearing; only the strength-of-verification label attached to them is corrected downward to match this project's own already-documented Ladder.
...
## Point 100 (2026-09-18) — C5 revival condition (a) checked against real literature via `boyko-specialist`: a non-empirical `E[θ(G_n)]` upper bound EXISTS (Bandeira et al. 2025, already used elsewhere in this project) but is many orders of magnitude too weak to reach `V_n=O(1/n)` via the `V_n≤2g_n/√n` route — closes C5's own named revival condition (a) as unreachable with current field knowledge, not merely untried [correction to author name and to scope, see Point 101]
...
**Found, `[VERIFIED]` via direct fetch of the primary source (not accepted from a search-engine summary alone):** Bandeira, Błasiok, Dmitriev, Faure, Kireeva & Kunisky, "The Lovász number of random circulant graphs" (arXiv:2502.16227) — the SAME paper this project already uses extensively for `E‖x*‖²=O(log³n)` and the `CV²` ceiling (Points 90/92) — proves, via a modified LP in the frequency domain plus the restricted isometry property (RIP) of the subsampled DFT matrix, **Theorem 1**: `√n ≤ E[θ(G)] ≤ C√(n·log log n)` for dense random circulant graphs (confirmed via `randomstrasse101.math.ethz.ch/posts/lovasz-circulant/`, citing the same arXiv ID this project already cites at lines 3189/8771/8807/9788/10045). A second, unrelated 2025 paper (Feige & Grinberg, arXiv:2506.02952, `[VERIFIED]` via direct fetch) targets ordinary Erdős–Rényi `G(n,1/2)` (not circulant graphs) and gives only a **conjectured**, not proven, bound (`E[θ]<1.55√n`, "heuristics and experiments," explicitly not rigorous) — checked and set aside as inapplicable to this project's circulant-graph model.
...
Кем: Bandeira et al. 2025 (arXiv:2502.16227), Theorem 1 -- proven, not conjectural.
...
## Point 101 (2026-09-18) — Correction to Point 100: citation misattribution fixed, and the "structurally unreachable" verdict narrowed to what was actually shown

**Trigger.** An external methodological review of Point 100 (relayed by the user) caught two real problems, both independently checked before accepting.

**Correction 1 — citation error, `[VERIFIED]` via direct fetch of the arXiv abstract page.** Point 100 attributed arXiv:2502.16227 to "Bandeira & Ferber" throughout (title, Phase 5 verdict, and its own propagation to `registry/graph.yaml`, `parked/H-CAT31-3-pol-proof-attempt.md`, `activeContext.md`, `tooling-eval/LEDGER.md` — 7 occurrences total, all introduced in this same session's Point 100, none pre-existing). **No author named "Ferber" exists on this paper.** The real author list, confirmed directly from arxiv.org/abs/2502.16227: **Afonso S. Bandeira, Jarosław Błasiok, Daniil Dmitriev, Ulysse Faure, Anastasia Kireeva, Dmitriy Kunisky.** This project's OWN earlier text (line 205-207 above, written before Point 100) already correctly names Ulysse Faure as a confirmed co-author of this exact paper — Point 100's "Ferber" contradicted this project's own prior, correct citation and should have been caught by cross-checking against it before writing. All 7 occurrences corrected in this same commit to "Bandeira, Błasiok, Dmitriev, Faure, Kireeva & Kunisky 2025" (or "Bandeira et al. 2025" for brevity, matching this project's own established shorthand at Points 90/92).

**Correction 2 — the Kill Analysis verdict overclaimed its own scope.** Point 100's stated conclusion — "closes C5's revival condition (a) as structurally unreachable via any general `E[θ]` bound, proven or conjectured" — is **too strong** relative to what was actually shown. Re-examined precisely: `V_n≤2g_n/√n` (Point 94) requires, for `V_n=O(1/n)`, that `g_n=O(n^{-1/2})`, equivalently `E[θ(G_n)]/√n=1+O(1/n)` — an explicit RELATIVE precision of `O(1/n)`, not merely "some vanishing relative error." What Point 100 actually demonstrated insufficient: (a) the PUBLISHED Theorem 1 (`E[θ]≤C√(n log log n)`, robust to any fixed `C` — the bound's SHAPE grows with `n`, so no value of `C` rescues it); (b) Conjecture 1 AS LITERALLY STATED (`E[θ]=(1+o(1))√n`, no explicit rate on the `o(1)` term) — demonstrated insufficient only under an assumed loose rate (`ε_n=1/ln n`) standing in for "conjecture proven, but with an unspecified, possibly slow, convergence rate." **Neither of these rules out a FUTURE result — a sharper form of the same conjecture proved WITH an explicit `O(1/n)`-relative (equivalently `g_n=O(n^{-1/2})`-absolute) rate would be exactly sufficient, and nothing in Point 100 shows such a result cannot exist.** The correct, narrower verdict: the currently published and currently conjectured (as stated, without a rate) results are insufficient by many orders of magnitude — this is real and load-bearing — but revival condition (a) is **sharpened, not closed**: it now requires specifically an `O(1/n)`-relative (or `g_n=O(n^{-1/2})`-absolute) estimate on `E[θ(G_n)]`, not merely "any non-empirical bound." This is a materially different, and more useful, target for a future search than Point 100's own blanket "unreachable" framing.

**Correction 3 — the specific numeric ratios (`163×`-`17372×` too loose) are `C`-dependent illustrations, not theorem-derived facts.** `c5_bandeira_bound_check.py` explicitly comments `C=1.0` as "order-1 constant, qualitative check — exact constant not given in abstract," and Point 100's own prose already flagged this ("C=1, qualitative"). The ROBUST claim — that the `V_n` bound derived from Theorem 1 GROWS with `n` rather than shrinking — holds for any fixed `C>0` (the shape of `√(log log n)` growth is `C`-independent). The SPECIFIC multiplicative factors quoted in Point 100/`activeContext.md`/`LEDGER.md` are illustrative under the placeholder `C=1`, not a precise consequence of the theorem — restated here explicitly so a future reader does not mistake them for an exact, `C`-verified computation.

**Independently re-verified before writing this correction:** re-ran `c5_bandeira_bound_check.py` unchanged — output matches Point 100's quoted numbers exactly (no arithmetic error in the original run, only a scope/attribution error in how the result was described and cited).

**Kill Analysis.** What is killed: the citation "Bandeira & Ferber" (never existed, corrected everywhere it appeared) and the overclaimed verdict "unreachable via any general `E[θ]` bound, proven or conjectured." What is NOT killed: the real, independently-verified finding that currently published/conjectured bounds are `163×`-`17372×` (under the `C=1` illustration) too coarse for `C0` — this remains a genuine, informative negative result, only now correctly scoped as "current results insufficient," not "no future result of this type could work."

**Corrected revival condition for C5, replacing Point 100's blanket closure:** a result — proven or, if conjectural, stated WITH an explicit rate — establishing `E[θ(G_n)]/√n=1+O(1/n)` specifically (equivalently `g_n=O(n^{-1/2})` in absolute terms). No such result is known to exist in the literature searched at Point 100; this remains open, not proven impossible.

**Artifacts:** none (documentation-only correction; `c5_bandeira_bound_check.py` re-run to confirm no arithmetic drift, output unchanged from Point 100).
