...
def betti1_total_persistence_series(
    x: np.ndarray, window: int, dim: int = EMBED_DIM, delay: int = EMBED_DELAY
) -> np.ndarray:
    """H-B3-1g: total persistence of H1 (sum of finite bar lengths) -- a DIFFERENT topological
    invariant than betti1_entropy_series. Entropy captures the SHAPE of the lifetime distribution
    (normalized, order-invariant to overall scale); total persistence captures the MAGNITUDE of
    topological structure (unnormalized, sensitive to how much "loop" structure exists overall).
    Named explicitly in H-B3-1f's decision.md as the next candidate: 'a different topological
    invariant' rather than another surrogate-null variant on the same entropy statistic."""
    out = np.full(len(x) - window + 1, np.nan)
    for i in range(len(out)):
        w = x[i : i + window]
        try:
...
def surrogate_null_curve(
    ...
    for backward compatibility with V1; pass `iaaft_surrogate` for V1'). `tda_stat_fn` (used only
    when kind=="betti") defaults to `betti1_entropy_series`; pass `betti1_total_persistence_series`
    for H-B3-1g -- the ONE thing that changes for that experiment (Minimal Relaxation Rule). Same
    length as the real tau series."""
    tda_stat_fn = tda_stat_fn or betti1_entropy_series
    rng = np.random.default_rng(seed)
    n_out = len(x) - window + 1
    ...
    for r in range(reps):
        ...
        if kind == "betti":
            stat = tda_stat_fn(surrogate, window)
        else:
