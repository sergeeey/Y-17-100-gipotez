# ruff: noqa
"""Thin driver: calls the blind implementer's r=3 library UNCHANGED for (31,4,13) and controls. Parameters and printing only."""
import json
import sys
import time
...
print("N2 B_13 := B_12:", brief(o), flush=True)
results["N2"] = o
Bs14, dims14 = build_tower(D, R, S + 1, p, seed)
o = pipeline(D, R, S + 1, p, Bs14)
print("N3 s=14 stage dims tail:", dims14[-3:], brief(o), flush=True)
o["stage_dims"] = dims14
results["N3"] = o

Path(HERE / "r4_results.json").write_text(json.dumps(results, indent=1, default=str), encoding="utf-8")
