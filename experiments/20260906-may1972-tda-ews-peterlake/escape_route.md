...
| Reference value | Lead time POSITIVE (TDA first) | Lead time ZERO (tie) | Lead time NEGATIVE (EWS first) |
|---|---|---|---|
| Peter Lake, per series | Candidate PASS for that series — check Paul Lake before promoting | REPEAT — try alternative topological summary (persistence entropy vs Betti-1) before concluding | This series does not support H; check remaining series before REJECT |
| Paul Lake (reference) | **Either method firing at all here is a FALSE POSITIVE** — that method is disqualified regardless of its Peter Lake result | Expected/correct outcome | Expected/correct outcome |

_WHY this shape instead of the generic in-band/out-of-band template: this experiment's outcome is a SIGN
(who leads) crossed with WHICH lake (should signal / should stay silent), not a magnitude vs threshold —
the generic per-sign table from `falsification-ladder.md`'s ceiling lesson still applies in spirit
(pre-register every combination), just instantiated for this experiment's actual outcome space._
