"""run.py — H-B3-1d (V1'): IAAFT surrogate-null detection rule, re-run on the same 9 series as V1.

Thin wrapper around the sibling V1 experiment's already-tested `cmd_run` -- reuses ALL of its
data loading, analysis, and verdict logic unchanged, passing only `surrogate_fn=iaaft_surrogate`
...
def cmd_run() -> dict:
    ...
    # (its historical REJECT result) -- caught before the first run, see
    # tests/test_v1prime_does_not_overwrite_v1_metrics.py.
    out = v1.cmd_run(
        surrogate_fn=v1.obrien.iaaft_surrogate,
        detection_rule_label="IAAFT-surrogate",
        write_output=False,
    )
