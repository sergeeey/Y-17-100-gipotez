"""run.py — H-B2-1f: does the Theorem-3.1/K_j=0 mechanism survive scaling from N=2 (H-B2-1e) to
N=8, keeping the same qualitative structure (non-normal, mixed-sign, "stiff" widely-separated
eigenvalues)?

A: 8x8 upper-triangular, diagonal = [0.5, -1, -2, -3, -4, -5, -6, -8] (one growing, seven
decaying, widely separated per the stiffness literature cited in H-B2-1d), fixed-seed random
off-diagonal coupling (non-normal).
"""
...
W = 0.5  # matches the growing eigenvalue, same choice as H-B2-1d/e
SEED = 0

_rng = np.random.default_rng(SEED)
A = np.diag(EIGENVALUES)
# Fixed-seed random upper-triangular coupling -- non-normal, same spirit as H-B2-1c/e's C=10
# coupling but scaled across all off-diagonal upper entries.
_coupling = _rng.uniform(-3.0, 3.0, size=(N_DIM, N_DIM))
...
def cmd_run() -> dict:
    ...
    out = {
        "config": {
            ...
            "w": W,
            "measured_m1": m1,
            "m1_at_n2_h_b2_1e": 3.806,
            "t": T,
            "n_values": N_VALUES.tolist(),
        },
        "results": results,
        "mechanism_holds_at_n8": all(r["all_n_bound_holds"] for r in results.values()),
    }
