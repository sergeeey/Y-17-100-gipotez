...
## Addendum (2026-09-06, later session) — upgraded to "different tool, same task" verification

Originally this experiment's positive control was a STATIC report file (`faure_cellcycle_attractors.md`)
generated by PyBoolNet at some point in the past — a strong but not maximal tier on the Independent
Verification Strength Ladder. While scoping a follow-up experiment on a larger network, installed
`pyboolnet` itself (`pip install pyboolnet`; confirmed it ships working Windows binaries for its
external solvers — `BNetToPrime`, `clasp`, `gringo` — under `pyboolnet/binaries/win64/`, verified by
listing the package's own files before trusting it) and ran it LIVE against the exact same `.bnet`
rules used in this experiment.

**Result: exact match.** `pyboolnet.attractors.compute_attractors()` reports `n_attractors=2`, steady
state `0000001011` (byte-identical to this experiment's own brute-force result and to the static
report), and a second, non-steady (complex) attractor. This upgrades the verification from "my code
matches a pre-computed report" to "my code matches a LIVE run of the actual third-party tool" —
**"different tool, same task,"** the strongest tier on the Independent Verification Strength Ladder
short of blind replication by another group (`falsification-ladder.md`). No change to the CONFIRMED
verdict; this only strengthens its evidentiary basis. Side benefit: confirms `pyboolnet` is usable in
this environment for larger networks where brute-force enumeration is infeasible (see `H-B7-4`).
