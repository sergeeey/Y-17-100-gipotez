"""run.py — H-B7-5: combined permanent do(RAS=1, TP53=0) two-hit perturbation on the Remy et al.
2015 bladder tumorigenesis network's bistable branch -- the deliberate two-assumption-changed
follow-up named in H-B7-4's own Relaxation Map, testing whether a biologically standard
oncogene+tumor-suppressor cooperating pair succeeds where RAS alone (H-B7-4) did not.

Reuses H-B7-1's pipeline functions (parse_bnet, compile_rules, synchronous_step), H-B7-2's
clamp_rule, and H-B7-3's run_until_attractor UNCHANGED via import -- all already generic, no
modification needed.
"""
...
# Same alphabetical-order convention as H-B7-4 (pyboolnet's compute_attractors state strings).
...
def apply_combined_clamp(compiled_rules: dict, clamps: dict[str, bool]) -> dict:
    """Apply clamp_rule once per (node, value) pair in `clamps`, sequentially -- a thin,
    explicit helper so the combined-clamp construction is a single named function, tested
    directly against a manual double clamp_rule call in
    tests/test_remy_tumorigenesis_twohit.py."""
    result = dict(compiled_rules)
    for node, value in clamps.items():
        result = h2.clamp_rule(result, node, value)
    return result
...
def cmd_run() -> dict:
    ...
    if final_pyboolnet_order_str == GROWTH_ARREST_STATE:
        final_label = "GROWTH_ARREST (unchanged -- two-hit insufficient)"
    elif final_pyboolnet_order_str == PROLIFERATION_STATE:
        final_label = "PROLIFERATION (flipped! cooperativity confirmed)"
    else:
        final_label = "OTHER/NEW attractor (not seen in H-B7-4's single-hit run)"
    ...
    out = {
        ...
        "comparison_to_h_b7_4_single_hit_ras": (
            "H-B7-4 (RAS alone): REJECTED, returned to Growth_arrest"
        ),
    }
