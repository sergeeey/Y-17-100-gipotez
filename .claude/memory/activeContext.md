...
**[VERIFIED — 2026-09-10, ADR-101] H-B7-19 (второй клэмп-узел p21CIP, exhaustive 16-строчное возмущение через generic table-based evaluator): ROBUST.** 15 ROBUST / 0 FRAGILE / 1 CRITERION_INVALID (переход отсутствует под `flip_FTFT`) — резкая асимметрия с RBL2 (H-B7-17: 2/3 robust, 1 fragile). Реальный баг пойман ДО запуска (не reviewer'ом): черновик table-override во время клэмп-фазы игнорировал бы собственную константу клэмпа для p21CIP — исправлено, закреплено regression-тестом с заведомо неверным override. Substrate-проверка прошла (орбита 12 состояний идентична во всех 17 условиях), baseline точно совпал с H-B7-15/17. FL Step 8a: reviewer сам разумно сузил scope до 2 представительных строк вместо всех 16, честно отметив это как sample-проверку — принято достаточным вместе с исчерпывающей (16/16) собственной проверкой построения таблицы. `graph.yaml`: 136 узлов, 168 рёбер, статус `confirmed`. Завершает возмущение обоих клэмп-узлов; `CyclinE1` (32 строки) остаётся единственной непроверенной целью.

**[VERIFIED — 2026-09-10, ADR-100] H-B7-18 (продолжение открытого вопроса из H-B7-17): почему flip_TF_drop_CyclinE1 упраздняет эскейп целиком — CONFIRMED-SELF-DESTABILIZING.** Mechanism Claim Gate, вывод ДО кода: при PROLIFERATION_STATE/_2 CyclinD1=False, CyclinE1=True — собственное RBL2=False состояния согласовано с исходным правилом, но НЕ с возмущённым (`!CyclinD1`→True) — предсказание: состояние перестаёт быть неподвижной точкой. Подтверждено точно: один синхронный шаг меняет РОВНО ОДИН узел (RBL2) на обеих ветках; дестабилизированная траектория сходится в ГЕНУИННО НОВУЮ GROWTH_ARREST-неподвижную точку, не совпадающую с уже известными константами. Независимый reviewer подтвердил оба под-утверждения + дополнительно проверил корректность подстановки символов через 3 раздельных объекта BooleanAlgebra в pipeline. `graph.yaml`: 135 узлов, 166 рёбер, статус `confirmed`. Закрывает вопрос из Revival Condition H-B7-17.
